...
final class HTreeDirectory<K, V> {
		...
		if (child_recid == 0) {
			...
			if (node instanceof HTreeDirectory) {
				...
				if (bucket.hasRoom()) {
					...
					HTreeDirectory dir = new HTreeDirectory(tree,
							(byte) (_depth + 1));
					long dir_recid = tree.db
							.insert(dir, tree.SERIALIZER, false);
	...
	public void defrag(DBStore r1, DBStore r2) throws IOException,
		...
		for (long[] sub : _children) {
			...
			for (long child : sub) {
				...
				Object t = tree.SERIALIZER
						.deserialize(new DataInputOutput(data));
	...
	class HDIterator<A> implements Iterator<A> {
		 ...
		 * 
		 * @param iterateKeys
		 *            True if iteration supplies keys, False if iterateKeys
		 *            supplies values.
		 */
		 ...
		 * enumerate. If no following bucket is found, the next Enumeration is
		...
		private void prepareNext() throws IOException {
			...
			do {
				...
				if (_child >= MAX_CHILDREN) {
					...
					// try next node
					_dir = (HTreeDirectory) _dirStack
							.remove(_dirStack.size() - 1);
					_child = ((Integer) _childStack
							.remove(_childStack.size() - 1)).intValue();
					continue;
				}
		...
		public void remove() {
			...
			// TODO current delete behaviour may change node layout. INVESTIGATE
			// if this can happen!
			tree.remove(last);
			last = null;
			expectedModCount++;
