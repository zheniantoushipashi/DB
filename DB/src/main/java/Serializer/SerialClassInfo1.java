...
public class SerialClassInfo1 {
    static  final  Serializer<ArrayList<ObjectClassInfo>> serializer = new Serializer<ArrayList<ObjectClassInfo>>(){
    	public  void serialize(DataOutput out, ArrayList<ObjectClassInfo> obj) throws IOException{
    		...
    		for(ObjectClassInfo oci :obj){
    			...
    			if(oci.isExternalizable) continue; //no fields
     ...
     public  ArrayList<ObjectClassInfo>  deserialize(DataInput in) throws IOException ,ClassNotFoundException{
    	  int  size = LongPacker.unpackInt(in);
    	  ArrayList<ObjectClassInfo> ret = new ArrayList<ObjectClassInfo>(size);
    	  for(int i = 0; i<size; i++){
    		  ...
    		  ret.add(new ObjectClassInfo(className, fields, isEnum, isExternalizable));
    	  }
    	  return  ret;
     }
   ...
   public  void registerClass(Class  clazz) throws IOException{
	   ...
	   ObjectClassInfo i = new ObjectClassInfo(clazz.getName(), fields,clazz.isEnum(), Externalizable.class.isAssignableFrom(clazz));
   ...
   public void setFieldValue(String fieldName, Object object, Object value) {
		...
		ObjectClassInfo classInfo = registered.get(classToclassId.get(object.getClass()));
