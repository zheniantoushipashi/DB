...
final class PageTransactionManager {
     ...
     * of hassle.
     */
    private ArrayList<PageBuffer> txn = new ArrayList<PageBuffer>();
    ...
    private void synchronizeLogFromMemory() throws IOException {
        ...
        TreeSet<PageBuffer> pageList = new TreeSet<PageBuffer>(PAGE_IO_COMPARTOR);
        ...
        if(txn!=null){
            ...
            for (Iterator<PageBuffer> k = txn.iterator(); k.hasNext(); ) {
                PageBuffer page = k.next();
    ...
    private void recover() throws IOException {
        ...
        while (true) {
            ArrayList<PageBuffer> pages = null;
            try {
                int size = LongPacker.unpackInt(ois);
                pages = new ArrayList<PageBuffer>(size);
                for (int i = 0; i < size; i++) {
                    PageBuffer b = new PageBuffer();
     ...
     */
    private void synchronizePages(Iterable<PageBuffer> pages, boolean fromCore)
            throws IOException {
        // write pages vector elements to the data file.
        for (PageBuffer cur : pages) {
            owner.synch(cur);
            if (fromCore) {
     ...
     */
    private void setClean(ArrayList<PageBuffer> pages)
            throws IOException {
        for (PageBuffer cur : pages) {
            cur.setClean();
        }
     ...
     */
    private void discardPages(ArrayList<PageBuffer> pages)
            throws IOException {
        for (PageBuffer cur : pages) {
     ...
     */
    void add(PageBuffer page) throws IOException {
...
//        oos = new DataOutputStream(new BufferedOutputStream(fos));
     ...
     * to write for this transaction.  The PageBuffer objects are ordered by
     * their page ids.
     */
    private static final Comparator<PageBuffer> PAGE_IO_COMPARTOR = new Comparator<PageBuffer>() {

        public int compare(PageBuffer page1, PageBuffer page2) {
