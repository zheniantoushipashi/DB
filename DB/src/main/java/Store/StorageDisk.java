...
class StorageDisk implements Storage {
	...
	public StorageDisk(String fileName, boolean readonly,
	...
	public void write(long pageNumber, ByteBuffer data) throws IOException {
		...
		// if (lastPageNumber + 1 != pageNumber) //TODO cache position again, so
	...
	public ByteBuffer read(long pageNumber) throws IOException {
		...
		while (remaining > 0) {
			...
			if (read == -1) {
				System.arraycopy(CLEAN_DATA, 0, buffer.array(), pos, remaining);
				break;
			}
			remaining -= read;
			pos += read;
	...
	public DataOutputStream openTransactionLog() throws IOException {
		...
		return new DataOutputStream(new BufferedOutputStream(fileOut)) {
			...
			// so we use little workaround to make sure that data were really
			// flushed
	...
	public DataInputStream readTransactionLog() {
		...
		try {
			ois = new DataInputStream(new BufferedInputStream(
